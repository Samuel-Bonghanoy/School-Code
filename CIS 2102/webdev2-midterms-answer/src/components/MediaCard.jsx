...
export default function MediaCard(props) {
  ...
  const { setBus1Passengers, setBus2Passengers, setBus3Passengers} = useBuses()
  ...
  return (
    <Card sx={{ maxWidth: 445, width: 250, ':hover': {
      boxShadow: 20,
    }  }}>
      <CardMedia
        sx={{ height: 180 }}
      ...
      <CardContent className='flex flex-col justify0-center items-center gap-3'>
        ...
        <div className='flex flex-wrap justify-center gap-1'>
           {destinations.map((d) => <Button onClick={() => handleClick(d)} variant="contained" key={d.id} >{d.destination}</Button> )}
        </div>
      </CardContent>
